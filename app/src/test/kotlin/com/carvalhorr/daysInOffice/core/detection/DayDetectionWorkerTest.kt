...
class DayDetectionWorkerTest {
    ...
    @Test
    fun `given Saturday at 9am when shouldDetect then returns true (detection runs 7 days)`() {
        val saturday = LocalDate.of(2026, 5, 23)
        assertTrue(DayDetectionWorker.shouldDetect(saturday, LocalTime.of(9, 0)))
    }

    @Test
    fun `given Sunday at 9am when shouldDetect then returns true (detection runs 7 days)`() {
        val sunday = LocalDate.of(2026, 5, 24)
        assertTrue(DayDetectionWorker.shouldDetect(sunday, LocalTime.of(9, 0)))
    }
    ...
    @Test
    fun `given Saturday at 11pm when shouldDetect then returns false (outside working hours)`() {
        val saturday = LocalDate.of(2026, 5, 23)
        assertFalse(DayDetectionWorker.shouldDetect(saturday, LocalTime.of(23, 0)))
    }
    ...
    @Test
    fun `given force_run false when Saturday at 10pm then shouldRun returns false (outside hours)`() {
        val saturday = LocalDate.of(2026, 5, 23)
    ...
    @Test
    fun `given force_run false when Saturday in working hours then shouldRun returns true`() {
        val saturday = LocalDate.of(2026, 5, 23)
        assertTrue(DayDetectionWorker.shouldRun(false, saturday, LocalTime.of(10, 0)))
    }
}
