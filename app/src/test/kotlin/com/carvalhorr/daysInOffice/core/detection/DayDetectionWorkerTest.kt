...
class DayDetectionWorkerTest {
    ...
    fun `given Monday at 9am when shouldDetect then returns true`() {
        ...
        assertTrue(DayDetectionWorker.shouldDetect(monday, LocalTime.of(9, 0), weekdays))
    }

    @Test
    fun `given Friday at 6pm when shouldDetect then returns true`() {
        val friday = LocalDate.of(2026, 5, 22)
        assertTrue(DayDetectionWorker.shouldDetect(friday, LocalTime.of(18, 0), weekdays))
    }

    @Test
    fun `given Saturday at 9am when shouldDetect then returns false`() {
        val saturday = LocalDate.of(2026, 5, 17)
        assertFalse(DayDetectionWorker.shouldDetect(saturday, LocalTime.of(9, 0), weekdays))
    }

    @Test
    fun `given Sunday at 9am when shouldDetect then returns false`() {
        val sunday = LocalDate.of(2026, 5, 16)
        assertFalse(DayDetectionWorker.shouldDetect(sunday, LocalTime.of(9, 0), weekdays))
    }

    @Test
    fun `given weekday at exactly 7am when shouldDetect then returns true`() {
        val monday = LocalDate.of(2026, 5, 18)
        assertTrue(DayDetectionWorker.shouldDetect(monday, LocalTime.of(7, 0), weekdays))
    }

    @Test
    fun `given weekday at exactly 7pm when shouldDetect then returns true`() {
        val monday = LocalDate.of(2026, 5, 18)
        assertTrue(DayDetectionWorker.shouldDetect(monday, LocalTime.of(19, 0), weekdays))
    }

    @Test
    fun `given weekday one minute before 7am when shouldDetect then returns false`() {
        val monday = LocalDate.of(2026, 5, 18)
        assertFalse(DayDetectionWorker.shouldDetect(monday, LocalTime.of(6, 59), weekdays))
    }

    @Test
    fun `given weekday one minute after 7pm when shouldDetect then returns false`() {
        val monday = LocalDate.of(2026, 5, 18)
        assertFalse(DayDetectionWorker.shouldDetect(monday, LocalTime.of(19, 1), weekdays))
    }

    @Test
    fun `given Wednesday at noon when shouldDetect then returns true`() {
        val wednesday = LocalDate.of(2026, 5, 20)
        assertTrue(DayDetectionWorker.shouldDetect(wednesday, LocalTime.of(12, 0), weekdays))
    }

    @Test
    fun `given force_run true when Saturday at 10pm then shouldRun returns true`() {
        val saturday = LocalDate.of(2026, 5, 16)
        assertTrue(DayDetectionWorker.shouldRun(true, saturday, LocalTime.of(22, 0), weekdays))
    }

    @Test
    fun `given force_run false when Saturday at 10pm then shouldRun returns false`() {
        val saturday = LocalDate.of(2026, 5, 16)
        assertFalse(DayDetectionWorker.shouldRun(false, saturday, LocalTime.of(22, 0), weekdays))
    }

    @Test
    fun `given force_run true when weekday before 7am then shouldRun returns true`() {
        val monday = LocalDate.of(2026, 5, 18)
        assertTrue(DayDetectionWorker.shouldRun(true, monday, LocalTime.of(6, 0), weekdays))
    }

    @Test
    fun `given force_run false when weekday in working hours then shouldRun returns true`() {
        val monday = LocalDate.of(2026, 5, 18)
        assertTrue(DayDetectionWorker.shouldRun(false, monday, LocalTime.of(9, 0), weekdays))
    }

    @Test
    fun `given Sat in workingDays when shouldDetect on Saturday then returns true (custom working week)`() {
        val saturday = LocalDate.of(2026, 5, 16)
        val mondayThruSaturday = weekdays + DayOfWeek.SATURDAY
        assertTrue(DayDetectionWorker.shouldDetect(saturday, LocalTime.of(10, 0), mondayThruSaturday))
    }
}
