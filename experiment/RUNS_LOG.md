...
- ~~**Detect Claude CLI rate-limit / quota exhaustion as a non-retryable error and pause the run instead of burning all 10 attempts.**~~ **DONE 2026-05-18** — implemented via canary + auto-wait. `experiment/scripts/check_claude_credit.sh` is a minimal canary that distinguishes rate-limit from generic errors. `run_experiment.sh` runs it (a) pre-flight at startup (aborts before any tasks are attempted if quota is exhausted), and (b) after every tool error during the task loop (if canary confirms rate-limit, polls every 5 min until credit returns — capped at 12h — then re-invokes the same attempt without counting it against the retry budget). Catches the rate-limit-cascade pattern observed twice in earlier runs (TASK-021 2026-05-17 and TASK-019 2026-05-18, ~835 s of TOOL_ERROR wasted per occurrence).
  - **Still TODO:** capture `attempt*_session.json` size / content even on failure so we can distinguish "model failed mid-conversation" from "CLI failed before starting" without needing the canary as a tie-breaker.
